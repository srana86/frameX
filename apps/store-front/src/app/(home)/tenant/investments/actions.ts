...
export async function getInvestments(
  ...
  try {
    ...
    const investmentsCol = await getTenantCollectionForAPI<Investment>("investments");
...
export async function createInvestment(data: { key: string; value: number; category?: string; notes?: string }): Promise<Investment> {
  try {
    const { getTenantCollectionForAPI, getTenantIdForAPI } = await import("@/lib/api-helpers");
    const investmentsCol = await getTenantCollectionForAPI<Investment>("investments");
...
export async function updateInvestment(
  ...
  try {
    const { getTenantCollectionForAPI } = await import("@/lib/api-helpers");
    const { ObjectId } = await import("@/lib/api-helpers");
    const investmentsCol = await getTenantCollectionForAPI<Investment>("investments");
    const { buildTenantQuery } = await import("@/lib/api-helpers");
    const baseQuery = await buildTenantQuery();
...
export async function getBudgets(
  ...
  try {
    const skip = (page - 1) * limit;
    const budgetsCol = await getTenantCollectionForAPI<Budget>("budgets");
    const investmentsCol = await getTenantCollectionForAPI("investments");
    const baseQuery = await buildTenantQuery();
