...
export async function getInventoryTransactions(page: number = 1, limit: number = 30, productId?: string): Promise<TransactionsResponse> {
  try {
    const skip = (page - 1) * limit;
    const transactionsCol = await getTenantCollectionForAPI("inventory_transactions");
    const baseQuery = await buildTenantQuery();
    let query: any = { ...baseQuery };
...
export async function adjustInventoryStock(
  ...
  try {
    const { getTenantIdForAPI, isUsingSharedDatabase } = await import("@/lib/api-helpers");
    const productsCol = await getTenantCollectionForAPI("products");
    const transactionsCol = await getTenantCollectionForAPI("inventory_transactions");
    const baseQuery = await buildTenantQuery();
