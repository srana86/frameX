...
export function EmailSettingsClient() {
  ...
  const loadSettings = async () => {
    setLoading(true);
    try {
      const data = await apiRequest<EmailProviderSettings>("GET", "/tenant/email-settings");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const saved = await apiRequest<EmailProviderSettings>("PUT", "/tenant/email-settings", payload);
  ...
  const handleTest = async (providerId: string) => {
    ...
    try {
      ...
      const connectionData = await apiRequest<any>("POST", "/tenant/email-settings", { action: "test", providerId });
      ...
      const emailData = await apiRequest<any>("POST", "/tenant/email-settings", { action: "send-test", providerId, to: testEmail });
