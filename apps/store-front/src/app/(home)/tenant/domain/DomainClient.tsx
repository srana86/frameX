...
import type { TenantDeployment } from "@/lib/tenant-types";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function DomainClient({ domainConfig: initialConfig, deployment }: DomainClientProps) {
  ...
  const refreshStatus = async () => {
    setRefreshing(true);
    try {
      const data = await apiRequest<any>("GET", "/tenant/domain/configure");
      if (data.domain) {
  ...
  const handleAddDomain = async () => {
    ...
    setLoading(true);
    try {
      await apiRequest<any>("POST", "/tenant/domain/configure", { domain: domain.toLowerCase().trim() });
  ...
  const handleCheckStatus = async () => {
    ...
    setChecking(true);
    try {
      const data = await apiRequest<any>("POST", "/tenant/domain/verify", { domain: domainConfig.domain });
  ...
  const handleRemoveDomain = async () => {
    ...
    setRemoving(true);
    try {
      await apiRequest<any>("DELETE", `/tenant/domain/remove?domain=${encodeURIComponent(domainConfig.domain)}`);
