...
import { Menu, ShoppingCart, User, Heart, LogOut, LayoutDashboard, Package, Home, X } from "lucide-react";
    ...
    <header className='sticky top-0 z-40 border-b bg-background'>
        ...
        >
          <div className='mx-auto max-w-[1440px] flex items-center justify-center gap-2 flex-wrap'>
            ...
            {banner.link && banner.linkText && (
              <Link href={banner.link} className='underline font-medium hover:opacity-80 transition-opacity'>
                {banner.linkText}
              </Link>
            )}
          </div>
      ...
      <div className='mx-auto flex h-14 sm:h-16 max-w-[1440px] items-center justify-between px-3 sm:px-4 md:px-6 lg:px-8 gap-2'>
        ...
        <div className='shrink-0 min-w-0'>
          <Logo brandConfig={brandConfig} className='scale-90 sm:scale-100 origin-left' />
        </div>
        ...
        <div className='hidden md:flex items-center gap-3'>
          ...
          <Button variant='ghost' size='icon' className='h-9 w-9 relative group' asChild>
            <Link href='/cart'>
              <ShoppingCart className={`w-4 h-4 transition-transform ${count > 0 ? "group-hover:scale-110" : ""}`} />
              {count > 0 && (
      ...
      <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
        <SheetContent side='left' className='w-[85vw] sm:w-[400px] p-0 flex flex-col [&>button]:hidden'>
          {/* Header with Logo */}
          <SheetHeader className='px-6 py-5 border-b bg-gradient-to-br from-primary/5 via-background to-background relative'>
            ...
            {brandConfig.brandTagline && (
              <p className='text-xs text-muted-foreground font-medium uppercase tracking-wide mt-1'>{brandConfig.brandTagline}</p>
            )}
          </SheetHeader>
              ...
              <>
                  ...
                  <div className='px-6 py-4 border-b bg-gradient-to-r from-primary/5 to-transparent'>
                    <div className='flex items-center gap-3'>
                      ...
                      <div className='flex-1 min-w-0'>
                        ...
                        {userRole && <div className='text-xs text-muted-foreground mt-0.5 capitalize'>{userRole}</div>}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className='px-4 py-3 space-y-1'>
                  ...
                  <Button
                    ...
                    asChild
                  >
                    <Link href='/account/orders' onClick={handleMobileLinkClick}>
