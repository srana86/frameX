...
export default async function OrderDetailsPage({ params }: Props) {
  ...
  const { getTenantCollectionForAPI, buildTenantQuery } = await import("@/lib/api-helpers");
  const { ObjectId } = await import("@/lib/api-helpers");
  const { loadTenantCollectionData } = await import("@/lib/tenant-data-loader");
  ...
  try {
    const orders = await loadTenantCollectionData<any>("orders", {}, { sort: { _id: -1 } });
  ...
  let products: any[] = [];
  try {
    const productsData = await loadTenantCollectionData<any>("products");
