...
export function DashboardClient({ initialData, brandName = "E-Commerce Store" }: Props) {
  ...
  // Get tenant ID
  useEffect(() => {
    apiRequest<any>("GET", "/tenant/context")
      .then((data) => {
            ...
            <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4'>
              ...
              <div className='flex flex-wrap gap-1.5 sm:gap-2 items-center'>
                ...
                <Button variant='secondary' size='sm' asChild>
                  <Link href='/tenant/profit-analysis'>Profit Analysis</Link>
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className='px-3 sm:px-4 pt-3 md:pt-4'>
            <div className='flex items-center justify-between gap-2 md:gap-3'>
              ...
              <Button className='rounded-full px-5' variant='outline' size='sm'>
                <Link href='/tenant/orders'>More</Link>
              </Button>
            </div>
          </div>
        ...
        <div className='px-3 sm:px-4 py-3'>
          <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4'>
            ...
            <Button variant='outline' size='sm' asChild className='px-5 rounded-full'>
              <Link href='/tenant/orders' className=' text-sm'>
                View All
              </Link>
            </Button>
          </div>
            ...
            <>
              ...
              <div className='md:hidden space-y-2.5 p-3'>
                {recentOrders.map((order) => (
                  <Link key={order.id} href={`/tenant/orders/${order.id}`} className='block'>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align='end' className='border-slate-200 dark:border-slate-800'>
                              <DropdownMenuItem asChild>
                                <Link href={`/tenant/orders/${order.id}`}>View Details</Link>
                              </DropdownMenuItem>
