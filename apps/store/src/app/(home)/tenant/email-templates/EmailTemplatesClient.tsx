...
export function EmailTemplatesClient() {
  ...
  const loadTemplates = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiRequest<any>("GET", "/tenant/email-templates");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await apiRequest<EmailTemplate>("PUT", "/tenant/email-templates", payload);
  ...
  const handleTestSend = async () => {
    ...
    try {
      const { html, design } = await exportCurrentDesign();
      const data = await apiRequest<any>("POST", "/tenant/email-templates", {
        action: "test",
        event: selectedEvent,
