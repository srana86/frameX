...
export async function GET(request: Request) {
  try {
    ...
    if (!zoneId) {
      return NextResponse.json({ error: "zone_id parameter is required" }, { status: 400 });
    }
    ...
    // Fetch areas from Pathao API
    const res = await fetch(`${PATHAO_BASE_URL}/aladdin/api/v1/zones/${encodeURIComponent(zoneId)}/area-list`, {
      method: "GET",
      headers: {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Failed to fetch Pathao areas: ${errorText || res.statusText}`);
    }

    const data = await res.json();

    // Extract areas from response: data.data.data
    const areas = data?.data?.data || [];

    return NextResponse.json({ areas });
  } catch (error: any) {
    console.error("GET /api/pathao/areas error:", error);
    if (error.message === "NEXT_REDIRECT") {
      throw error;
    }
    return NextResponse.json({ error: error?.message || "Failed to fetch Pathao areas" }, { status: 500 });
  }
}
