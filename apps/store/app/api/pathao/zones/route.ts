...
export async function GET(request: Request) {
  try {
    ...
    if (!cityId) {
      return NextResponse.json({ error: "city_id parameter is required" }, { status: 400 });
    }
    ...
    // Fetch zones from Pathao API
    const res = await fetch(`${PATHAO_BASE_URL}/aladdin/api/v1/cities/${encodeURIComponent(cityId)}/zone-list`, {
      method: "GET",
      headers: {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Failed to fetch Pathao zones: ${errorText || res.statusText}`);
    }

    const data = await res.json();

    // Extract zones from response: data.data.data
    const zones = data?.data?.data || [];

    return NextResponse.json({ zones });
  } catch (error: any) {
    console.error("GET /api/pathao/zones error:", error);
    if (error.message === "NEXT_REDIRECT") {
      throw error;
    }
    return NextResponse.json({ error: error?.message || "Failed to fetch Pathao zones" }, { status: 500 });
  }
}
