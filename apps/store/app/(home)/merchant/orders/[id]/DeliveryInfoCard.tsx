              ...
              <div className='flex items-center justify-between'>
                <div className='flex-1 min-w-0'>
                  <p className='text-[10px] text-muted-foreground mb-0.5'>
                    Order ID {order.courier?.merchantOrderId ? "(Sent to Courier)" : ""}
                  </p>
                  <p className='text-sm font-mono font-bold'>{order.courier?.merchantOrderId || displayOrderId}</p>
                </div>
                <Button
                  variant='ghost'
                  size='sm'
                  onClick={() => onCopyToClipboard(order.courier?.merchantOrderId || displayOrderId, "Order ID")}
                  className='h-8 w-8 p-0 shrink-0'
                >
                  <Copy className='h-3.5 w-3.5' />
                </Button>
              </div>
              {order.courier?.merchantOrderId && <p className='text-[9px] text-muted-foreground mt-1'>This ID is used with the courier</p>}
            </div>
            ...
            {hasDelivery && (
              <div className='space-y-2'>
                ...
                {order.courier?.serviceId === "pathao" && order.customer.phone && (
                  <a
                    href={`https://merchant.pathao.com/tracking?consignment_id=${encodeURIComponent(
                      consignmentId
        ...
        <div className='p-2.5 sm:p-3 bg-gradient-to-r from-primary/5 to-primary/10 rounded-lg border border-primary/20 space-y-2'>
          ...
          <div className='flex items-center gap-2'>
            <Badge variant='outline' className='font-mono text-sm px-3 py-1 bg-background border-primary/30'>
              {order.courier?.merchantOrderId || displayOrderId}
            </Badge>
            <Button
              variant='ghost'
              size='sm'
            ...
            {showAllDetails && (
              <div className='space-y-2 pt-2 border-t'>
                <div className='grid grid-cols-1 gap-2 text-xs'>
                  {/* Merchant Order ID - The ID you sent to the courier */}
                  {order.courier.merchantOrderId && (
                    <div className='space-y-0.5'>
                      ...
                      <div className='flex items-center gap-1'>
                        ...
                        <Button
                          variant='ghost'
                          size='sm'
                          onClick={() => onCopyToClipboard(order.courier!.merchantOrderId!, "Merchant Order ID")}
