...
import type {
  CustomerFraudData,
  UsageStats,
} from "@/lib/fraud-check/fraudshield-api";
import {
  getRiskBadgeColor,
  calculateRiskLevel,
  type FraudRiskLevel,
} from "@/lib/fraud-check/common";
...
function CourierHistoryItem({
  courier,
  index,
}: {
  courier: CourierHistory;
  index: number;
}) {
  const [imageError, setImageError] = useState(false);
  const successRate =
    courier.total > 0
      ? Math.round((courier.successful / courier.total) * 100)
      : 0;
...
function transformAPIResponse(
  ...
  const fraud_risk: FraudRiskLevel =
  ...
  const courier_history = Object.entries(response.courierData)
    ...
    ); // Only include couriers with parcels
...
export function FraudCheckClient() {
  ...
  const [customerData, setCustomerData] = useState<CustomerFraudData | null>(
    null
  );
  ...
  const loadUsageStats = async () => {
    try {
      // API client unwraps the response, so data is the unwrapped payload
      const data = await api.get<
        UsageStats | { success: boolean; data: UsageStats }
      >("fraud-check");
      // Handle both unwrapped UsageStats and nested { success, data } format
      if (data && typeof data === "object") {
        if ("success" in data && "data" in data && (data as any).success) {
          setUsageStats((data as any).data);
        } else if ("today_usage" in data || "monthly_usage" in data) {
          // Direct UsageStats object
  ...
  const handleCheck = async () => {
    ...
    try {
      // API client unwraps the response, so data is the unwrapped payload
      // It can be either NewAPIResponse (with status and courierData) or CustomerFraudData
      const data = await api.post<
        | NewAPIResponse
        | CustomerFraudData
        | { success: boolean; data: CustomerFraudData; message?: string }
      >("fraud-check", { phone: phone.trim() });
      console.log(data);

      // Handle new API response structure (status and courierData)
      if (
        data &&
        typeof data === "object" &&
        "status" in data &&
      ...
      // Handle old API response structure (backward compatibility - nested success/data)
      else if (
        data &&
        typeof data === "object" &&
        "success" in data &&
        ...
        setCustomerData((data as any).data);
        toast.success("Customer data retrieved successfully");
        // Refresh usage stats
        loadUsageStats();
      }
      // Handle direct CustomerFraudData
      else if (data && typeof data === "object" && "phone" in data) {
        setCustomerData(data as CustomerFraudData);
        toast.success("Customer data retrieved successfully");
        // Refresh usage stats
        loadUsageStats();
      } else {
        const errorMessage =
          (data as any)?.message || "Failed to retrieve customer data";
  ...
  const loadDemoData = () => {
    ...
    setIsDemoMode(true);
    toast.success(
      "Demo data loaded successfully! This is sample data for preview."
    );
  };
    ...
    <div className="space-y-6 mt-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Check customer fraud risk by analyzing their courier delivery
              history across multiple services.
            </p>
          </div>
          <Button
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter a Bangladeshi phone number to check fraud risk
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {isDemoMode && (
            <Alert className="mt-3 border-blue-200 bg-blue-50/50">
              ...
              <AlertDescription className="text-blue-900">
                You&apos;re viewing demo data. Enter a real phone number and
                click &quot;Check&quot; to see actual customer data.
              </AlertDescription>
            </Alert>
          )}
      ...
      {customerData && (
        <div className="space-y-4">
          {/* Risk Level Gauge */}
          <Card className="bg-linear-to-br from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                ...
                <div className="relative w-64 h-64">
                  ...
                  >
                    ...
                    <circle
                      ...
                      strokeWidth="20"
                      strokeDasharray={`${
                        (customerData.success_rate / 100) * 502.65
                      } 502.65`}
                      strokeLinecap="round"
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-3xl font-bold text-green-600">
                    {customerData.successful_deliveries}
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-3xl font-bold text-red-600">
                    {customerData.failed_deliveries}
                  </p>
                </div>
              ...
              {customerData.last_delivery && (
                <>
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>
                      Last delivery: {formatDate(customerData.last_delivery)}
                    </span>
                  </div>
                </>
              )}
          ...
          {/* Courier History */}
          {customerData.courier_history &&
            customerData.courier_history.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Delivery statistics across different courier services
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {customerData.courier_history.map((courier, index) => (
                      <CourierHistoryItem
                        key={index}
                        courier={courier as CourierHistory}
                        index={index}
                      />
                    ))}
                  </div>
