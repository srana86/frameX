  ...
  if (step.status === "running") {
    return (
      <div className={`${baseClasses} h-10 w-10`}>
        ...
        <div
          className={`relative flex h-10 w-10 items-center justify-center rounded-full ${step.color} shadow-lg`}
        >
    ...
    const loadData = async () => {
      try {
        ...
        const [sessionData, settingsData] = await Promise.all([
          api.get<any>(`checkout/session?tranId=${tranId}`).catch(() => null),
  ...
  const runSimulation = async () => {
    ...
    try {
      ...
      updateStep(
        "1",
        "completed",
        `Payment verified: ${session.planName} Plan`
      );
      ...
      await api.post("merchants", {
        ...
        status: "trial",
        settings: {
          brandName: session.merchantName,
          currency: "USD",
          timezone: "UTC",
        },
      });
      updateStep("2", "completed", `Merchant: ${session.merchantName}`, {
        merchantId,
      });
      ...
      const plans = await api.get<any[]>("plans");
      const selectedPlan =
        plans.find((p: any) => p.id === session.planId) || plans[0];

      // Calculate billing period based on billing cycle (1, 6, or 12 months)
      const billingCycleMonths = Number(session.billingCycle) || 1;
      ...
      const subscriptionData = await api.post<any>("subscriptions", {
        id: subscriptionId,
        merchantId,
      ...
      const cycleLabel =
        billingCycleMonths === 1
          ...
          ? "6 Months"
          : "Yearly";
      updateStep("3", "completed", `${cycleLabel} subscription active`, {
        subscriptionId: actualSubscriptionId,
      });

      // Step 4: Create Database
      updateStep("4", "running", "Creating MongoDB database...");
      const dbData = await api.post<any>("simulate/create-database", {
        merchantId,
      });
      ...
      <section className="min-h-screen flex items-center justify-center bg-gradient-to-b from-white via-blue-50/30 to-white">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-gray-600 mb-6">
            We couldn&apos;t find your checkout session. Please try again.
          </p>
          <Link
            href="/#pricing"
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {!simulationStarted && (
          <div ref={headerRef} className="text-center mb-12">
            ...
            >
              Thank you for choosing FrameX! Click the button below to set up
              your store.
            </p>
