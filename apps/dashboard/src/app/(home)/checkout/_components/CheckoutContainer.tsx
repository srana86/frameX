...
function CheckoutContainerInner() {
  ...
  const [billingCycle, setBillingCycle] = useState<BillingCycleMonths>(initialCycle);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [plansRes, settingsRes] = await Promise.all([fetch("/api/plans?active=true"), fetch("/api/settings/general")]);
        ...
        if (plansRes.ok) {
          ...
          if (planId) {
            const plan = data.find((p: PlanInfo) => p.id === planId);
            if (plan) {
  ...
  const monthlyEquivalent = selectedPlan ? selectedPlan.price / (selectedPlan.billingCycleMonths || 1) : 0;
            ...
            <form onSubmit={handleSubmit} className='space-y-8'>
              ...
              >
                <div className='flex items-center gap-3 mb-6'>
                  ...
                  <div>
                    <h3 className='text-lg font-semibold text-gray-900' style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}>
                      Billing Period
                    </h3>
                    <p className='text-sm text-gray-500' style={{ fontFamily: "var(--font-urbanist), sans-serif" }}>
                      Choose how long you want to subscribe
                    </p>
                  </div>
                </div>

                <div className={cn("grid gap-3", availableCycles.length <= 3 ? `grid-cols-${availableCycles.length}` : "grid-cols-3")}>
                  {availableCycles.map((cycleMonths) => {
                    ...
                    const planForCycle = selectedPlan?.name
                      ? allPlans.find((p) => p.name === selectedPlan.name && p.billingCycleMonths === cycleMonths)
                      : allPlans.find((p) => p.billingCycleMonths === cycleMonths);
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected ? "border-blue-500 bg-blue-50/50" : "border-gray-200 hover:border-gray-300 bg-white"
                        )}
                      >
                        {cycle.discount > 0 && (
                        ...
                        <p
                          className={cn("font-semibold text-sm mb-1", isSelected ? "text-blue-700" : "text-gray-900")}
                          style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}
                        >
                          {cycle.label}
                        </p>
                        <p
                          className={cn("text-lg font-bold", isSelected ? "text-blue-600" : "text-gray-700")}
                          style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}
                        >
                          {currencySymbol}
                          {cyclePrice}
                        ...
                        {cycleMonths > 1 && cyclePrice > 0 && (
                          <p className='text-xs text-gray-500' style={{ fontFamily: "var(--font-urbanist), sans-serif" }}>
                            {currencySymbol}
              ...
              >
                {loading ? (
                  ...
                  <>
                    {displayPrice === 0 ? "Start Free" : `Pay ${currencySymbol}${displayPrice}`}
              ...
              <div className='flex items-center justify-center gap-2 text-sm text-gray-500'>
                ...
                <span style={{ fontFamily: "var(--font-urbanist), sans-serif" }}>Secured by SSLCommerz - 256-bit SSL encryption</span>
              </div>
            </form>
          </div>
            ...
            <div className='sticky top-24'>
                ...
                {selectedPlan ? (
                  <>
                    ...
                    <div className='flex items-center justify-between pb-4 border-b border-gray-100'>
                      <div>
                        <h4 className='text-xl font-semibold text-gray-900' style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}>
                          {selectedPlan.name}
                        </h4>
                        <p className='text-sm text-gray-500' style={{ fontFamily: "var(--font-urbanist), sans-serif" }}>
                          {getBillingCycleLabel(selectedPlan.billingCycleMonths)} subscription
                        </p>
                      </div>
                      <div className='text-right'>
                        <div className='flex items-baseline gap-1'>
                          <span className='text-3xl font-bold text-gray-900' style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}>
                            {currencySymbol}
                            {displayPrice}
                          </span>
                        </div>
                        {selectedPlan.billingCycleMonths > 1 && selectedPlan.price > 0 && (
                          <p className='text-xs text-gray-500' style={{ fontFamily: "var(--font-urbanist), sans-serif" }}>
                            {currencySymbol}
                    ...
                    {/* Features List */}
                    <div className='py-4'>
                      <p className='text-sm font-semibold text-gray-700 mb-3' style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}>
                        What's included:
                      </p>
                      <ul className='space-y-2.5'>
                        {(selectedPlan.featuresList || []).map((feature, index) => (
                          <li key={index} className='flex items-center gap-2.5'>
                            ...
                            <span className='text-sm text-gray-600' style={{ fontFamily: "var(--font-urbanist), sans-serif" }}>
                              {feature}
                            </span>
                          </li>
                        ))}
                    ...
                    <div className='pt-4 border-t border-gray-100'>
                      <div className='flex items-center justify-between'>
                        <span className='text-lg font-semibold text-gray-900' style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}>
                          Total Today
                        </span>
                        <span className='text-2xl font-bold text-[#0448FD]' style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}>
                          {currencySymbol}
                          {displayPrice}
                        </span>
                      </div>
              ...
              <div className='bg-gradient-to-br from-blue-50 to-purple-50 rounded-2xl p-6 border border-blue-100/50'>
                <div className='flex items-start gap-3'>
                  ...
                  <div>
                    <h4 className='text-sm font-semibold text-gray-900 mb-1' style={{ fontFamily: "var(--font-nunito-sans), sans-serif" }}>
                      What happens next?
                    </h4>
                    <ul className='text-xs text-gray-600 space-y-1.5' style={{ fontFamily: "var(--font-urbanist), sans-serif" }}>
