...
const STATUS_OPTIONS = [
  ...
  { value: "inactive", label: "Inactive", color: "bg-gray-500", icon: AlertTriangle },
];
...
function DeploymentHealthIndicator({ status }: { status: string }) {
  const config = STATUS_OPTIONS.find((s) => s.value === status) || STATUS_OPTIONS[3];
...
export default function DeploymentsPage() {
  ...
  const [viewingDeployment, setViewingDeployment] = useState<MerchantDeployment | null>(null);

  const loadData = async () => {
    setLoading(true);
    try {
      const [deploymentsRes, merchantsRes] = await Promise.all([fetch("/api/deployments"), fetch("/api/merchants")]);
  ...
  const stats = useMemo(() => {
    ...
    const pending = deployments.filter((d) => d.deploymentStatus === "pending").length;
    const failed = deployments.filter((d) => d.deploymentStatus === "failed").length;
    const customDomains = deployments.filter((d) => d.deploymentType === "custom_domain").length;
    const subdomains = deployments.filter((d) => d.deploymentType === "subdomain").length;

    const uptime = total > 0 ? ((active / total) * 100).toFixed(1) : "0";

    return { total, active, pending, failed, customDomains, subdomains, uptime };
  }, [deployments]);
  ...
  const filteredDeployments = useMemo(() => {
    return deployments.filter((deployment) => {
      ...
      const matchesSearch =
        ...
        deployment.deploymentUrl?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        deployment.merchantId.toLowerCase().includes(searchQuery.toLowerCase());

      const matchesStatus = statusFilter === "all" || deployment.deploymentStatus === statusFilter;
      const matchesType = typeFilter === "all" || deployment.deploymentType === typeFilter;
  ...
  const typeDistribution = useMemo(() => {
    const counts: Record<string, number> = {};
    deployments.forEach((d) => {
      const type = d.deploymentType === "custom_domain" ? "Custom Domain" : "Subdomain";
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>Monitor deployment health, status, and infrastructure</p>
        </div>
      ...
      <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
        <StatCard title='Total Deployments' value={stats.total} icon={Server} color='bg-primary' description={`${stats.active} active`} />
        <StatCard title='Uptime Rate' value={`${stats.uptime}%`} icon={Activity} color='bg-green-500' description='Active deployments' />
        <StatCard
        ...
        <StatCard
          ...
          icon={AlertTriangle}
          color={stats.pending + stats.failed > 0 ? "bg-yellow-500" : "bg-gray-400"}
                ...
                <TableBody>
                  {filteredDeployments.map((deployment) => (
                    <TableRow key={deployment.id} className='group'>
                      <TableCell>
                        <DeploymentHealthIndicator status={deployment.deploymentStatus} />
                      </TableCell>
                      <TableCell>
                        <div className='flex items-center gap-3'>
                          <div className='flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 font-semibold text-primary'>
                            {getMerchantName(deployment.merchantId).charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <p className='font-medium'>{getMerchantName(deployment.merchantId)}</p>
                            <p className='text-xs text-muted-foreground'>ID: {deployment.merchantId.slice(0, 12)}...</p>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <span className='text-sm'>{deployment.deploymentProvider || "N/A"}</span>
                      </TableCell>
                      <TableCell className='text-right'>
                        <div className='flex justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100'>
                          <Button variant='ghost' size='icon' onClick={() => handleViewDeployment(deployment)}>
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            ...
            <DialogDescription>Complete deployment information</DialogDescription>
          </DialogHeader>
          {viewingDeployment && (
            <div className='space-y-6'>
              <div className='flex items-center gap-4'>
                <div className='flex h-16 w-16 items-center justify-center rounded-full bg-primary/10 text-2xl font-bold text-primary'>
                  {getMerchantName(viewingDeployment.merchantId).charAt(0).toUpperCase()}
                </div>
                <div>
                  <h3 className='text-xl font-semibold'>{getMerchantName(viewingDeployment.merchantId)}</h3>
                  <p className='text-muted-foreground'>{viewingDeployment.deploymentUrl || "No URL"}</p>
                </div>
              ...
              <div className='grid gap-4 sm:grid-cols-2'>
                ...
                <div className='space-y-1'>
                  ...
                  <p className='font-mono text-sm'>{viewingDeployment.merchantId}</p>
                </div>
                <div className='space-y-1'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <span className='capitalize'>{viewingDeployment.deploymentType.replace("_", " ")}</span>
                  </div>
                </div>
                <div className='space-y-1'>
                  <Label className='text-xs text-muted-foreground'>Provider</Label>
                  <p>{viewingDeployment.deploymentProvider || "Not specified"}</p>
                </div>
                <div className='space-y-1'>
                ...
                {viewingDeployment.createdAt && (
                  <div className='space-y-1'>
                    ...
                    <p className='text-sm'>{new Date(viewingDeployment.createdAt).toLocaleString()}</p>
                  </div>
                )}
              </div>
              ...
              <div className='flex justify-end gap-2'>
                {viewingDeployment.deploymentUrl && (
                  <a href={`https://${viewingDeployment.deploymentUrl}`} target='_blank' rel='noopener noreferrer'>
