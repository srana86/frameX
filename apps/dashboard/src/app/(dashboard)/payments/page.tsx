...
export default function PaymentsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const thisMonthRevenue = thisMonth.filter((p) => p.status === "completed").reduce((sum, p) => sum + p.amount, 0);

    const lastMonthRevenue = lastMonth.filter((p) => p.status === "completed").reduce((sum, p) => sum + p.amount, 0);
  ...
  const exportPayments = () => {
    const csvContent = [
      ["Transaction ID", "Merchant", "Email", "Plan", "Amount", "Status", "Date"].join(","),
      ...filteredPayments.map((p) =>
        [p.tranId, p.merchantName, p.merchantEmail, p.planName, p.amount, p.status, new Date(p.createdAt).toLocaleDateString()].join(",")
      ),
    ].join("\n");
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>Track all payment transactions and revenue</p>
        </div>
      ...
      <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
        <StatCard
          ...
          trendValue={`${Math.abs(stats.revenueGrowth).toFixed(1)}% from last month`}
        />
        <StatCard
          title='This Month'
        ...
        <StatCard
          ...
          description={`${((stats.completed / (stats.total || 1)) * 100).toFixed(0)}% success rate`}
        />
        <StatCard
        ...
        <CardContent>
          {loading ? (
            ...
            <div className='py-12 text-center'>
              ...
              <p className='text-muted-foreground'>{payments.length === 0 ? "No payments yet" : "No payments match your filters"}</p>
            </div>
          ) : (
              ...
              <Table>
                ...
                <TableBody>
                  {filteredPayments.map((payment) => (
                    <TableRow key={payment.id || payment.tranId} className='group'>
                      <TableCell>
                        <div>
                          <p className='font-mono text-xs'>{payment.tranId}</p>
                          {payment.bankTranId && <p className='text-xs text-muted-foreground'>Bank: {payment.bankTranId}</p>}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className='flex items-center gap-3'>
                          <div className='flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 font-semibold text-primary'>
                            {(payment.merchantName || "?").charAt(0).toUpperCase()}
                          </div>
                          <div>
          ...
          <DialogContent className='max-w-2xl'>
            ...
            <div className='space-y-6'>
              <div className='flex items-center gap-4'>
                ...
                <div>
                  <h3 className='text-xl font-semibold'>{formatAmount(viewingPayment.amount)}</h3>
                  <p className='text-muted-foreground'>{viewingPayment.currency}</p>
                </div>
              ...
              <div className='grid gap-4 sm:grid-cols-2'>
                ...
                <div className='space-y-1'>
                  <Label className='text-xs text-muted-foreground'>Bank Transaction ID</Label>
                  <p className='font-mono text-sm'>{viewingPayment.bankTranId || "N/A"}</p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-xs text-muted-foreground'>Validation ID</Label>
                  <p className='font-mono text-sm'>{viewingPayment.valId || "N/A"}</p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-xs text-muted-foreground'>Payment Method</Label>
                  <p className='text-sm'>
                    {viewingPayment.cardType || viewingPayment.paymentMethod || "N/A"}
              ...
              <div className='grid gap-3 sm:grid-cols-2'>
                <div className='space-y-1'>
                  ...
                  <p className='text-sm'>{new Date(viewingPayment.createdAt).toLocaleString()}</p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-xs text-muted-foreground'>Updated At</Label>
                  <p className='text-sm'>{new Date(viewingPayment.updatedAt).toLocaleString()}</p>
                </div>
              </div>
