  ...
  return (
    <Card
      className={`transition-all ${
        check.status === "unhealthy" ? "border-red-500/50" : check.status === "degraded" ? "border-yellow-500/50" : ""
      }`}
    >
      <CardContent className='pt-6'>
          ...
          <div className='flex items-center gap-3'>
            <div
              className={`rounded-lg p-2 ${
                check.status === "healthy" ? "bg-green-500/10" : check.status === "degraded" ? "bg-yellow-500/10" : "bg-red-500/10"
              }`}
            >
        ...
        {check.latency !== undefined && (
          <div className='mt-4'>
            <div className='flex items-center justify-between text-sm'>
              ...
              <span className={check.latency > 1000 ? "text-yellow-600" : "text-green-600"}>{check.latency}ms</span>
            </div>
            <Progress
              value={Math.min(100, (check.latency / 2000) * 100)}
              className={`mt-1 h-1 ${check.latency > 1000 ? "[&>div]:bg-yellow-500" : "[&>div]:bg-green-500"}`}
            />
          </div>
        )}
        ...
        <p className='mt-3 text-xs text-muted-foreground'>Last checked: {new Date(check.lastChecked).toLocaleTimeString()}</p>
      </CardContent>
    </Card>
  );
}
...
export default function SystemHealthPage() {
  ...
  const overallStatusColor =
    health?.status === "healthy" ? "text-green-500" : health?.status === "degraded" ? "text-yellow-500" : "text-red-500";
    ...
    <div className='space-y-6'>
          ...
          <div className='flex flex-col items-center justify-center gap-4 sm:flex-row sm:justify-between'>
            <div className='flex items-center gap-4'>
              <div
                className={`rounded-full p-4 ${
                  health?.status === "healthy" ? "bg-green-500/20" : health?.status === "degraded" ? "bg-yellow-500/20" : "bg-red-500/20"
                }`}
              >
                <Activity className={`h-8 w-8 ${overallStatusColor}`} />
              </div>
              <div>
                <h2 className={`text-2xl font-bold ${overallStatusColor}`}>{loading ? "Checking..." : health?.message || "Unknown"}</h2>
                <p className='text-sm text-muted-foreground'>
                  {health?.timestamp ? `Last updated: ${new Date(health.timestamp).toLocaleString()}` : ""}
                </p>
              </div>
            </div>
            ...
            {health?.summary && (
              <div className='flex gap-6'>
                <div className='text-center'>
                  <p className='text-2xl font-bold text-green-500'>{health.summary.healthy}</p>
                  <p className='text-xs text-muted-foreground'>Healthy</p>
                </div>
                <div className='text-center'>
                  <p className='text-2xl font-bold text-yellow-500'>{health.summary.degraded}</p>
                  <p className='text-xs text-muted-foreground'>Degraded</p>
                </div>
                <div className='text-center'>
                  <p className='text-2xl font-bold text-red-500'>{health.summary.unhealthy}</p>
            ...
            {health?.totalCheckTime !== undefined && (
              <div className='text-center'>
                ...
                <p className='text-xs text-muted-foreground'>Total Check Time</p>
              </div>
            )}
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className='grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
          <Button variant='outline' className='justify-start' onClick={loadHealth}>
