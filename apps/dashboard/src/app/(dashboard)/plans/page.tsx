...
function PlanCard({
  ...
  return (
    <Card className={`relative overflow-hidden transition-all hover:shadow-lg ${plan.isPopular ? "ring-2 ring-primary" : ""}`}>
      {plan.isPopular && (
      ...
      <CardContent className='space-y-4'>
        <div>
          <div className='flex items-baseline gap-1'>
            <span className='text-4xl font-bold'>{formatAmount(plan.price)}</span>
            <span className='text-muted-foreground'>/{getBillingLabel(plan.billingCycleMonths).toLowerCase()}</span>
          </div>
          <p className='text-sm text-muted-foreground mt-1'>{formatAmount(monthlyEquivalent)}/month</p>
        </div>
          ...
          <div className='border-t pt-3'>
            <ul className='space-y-1 text-sm text-muted-foreground'>
              ...
              {plan.featuresList.length > 3 && (
                <li className='text-xs text-muted-foreground/70'>+{plan.featuresList.length - 3} more features</li>
              )}
            </ul>
          </div>
        )}
...
export default function PlansPage() {
  ...
  const [cycleFilter, setCycleFilter] = useState<"all" | "1" | "6" | "12">("all");

  const loadData = async () => {
    setLoading(true);
    try {
      const [plansRes, subsRes] = await Promise.all([fetch("/api/plans"), fetch("/api/subscriptions")]);
  ...
  const sortedPlans = useMemo(() => {
    return [...filteredPlans].sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
  }, [filteredPlans]);
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>Create separate plans for each billing cycle (monthly, 6-month, yearly)</p>
        </div>
      ...
      <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-5'>
        ...
        <Card>
          <CardContent className='pt-6'>
            <div className='flex items-center justify-between'>
              <div>
                <p className='text-sm font-medium text-muted-foreground'>Monthly Plans</p>
        ...
        <Card>
          <CardContent className='pt-6'>
            <div className='flex items-center justify-between'>
              <div>
                <p className='text-sm font-medium text-muted-foreground'>6-Month Plans</p>
        ...
        <Card>
          <CardContent className='pt-6'>
            <div className='flex items-center justify-between'>
              <div>
                <p className='text-sm font-medium text-muted-foreground'>Yearly Plans</p>
      ...
      <div className='flex flex-wrap items-center justify-between gap-4'>
        <Tabs value={cycleFilter} onValueChange={(v) => setCycleFilter(v as any)}>
          <TabsList>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className='py-12 text-center'>
            ...
            <p className='text-muted-foreground'>
              {cycleFilter === "all" ? "No plans found" : `No ${getBillingLabel(parseInt(cycleFilter))} plans found`}
            </p>
            <Link href='/plans/new'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Showing {sortedPlans.length} {cycleFilter === "all" ? "" : getBillingLabel(parseInt(cycleFilter)).toLowerCase()} plan(s)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {sortedPlans.map((plan) => (
                  <TableRow key={plan.id} className='group'>
                    ...
                    <TableCell>
                      <Badge className={getBillingBadgeColor(plan.billingCycleMonths)}>{getBillingLabel(plan.billingCycleMonths)}</Badge>
                    </TableCell>
                    <TableCell>
                      <div>
                        <span className='font-semibold'>{formatAmount(plan.price)}</span>
                        <span className='text-xs text-muted-foreground ml-1'>
                          ({formatAmount(plan.price / plan.billingCycleMonths)}/mo)
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant='outline'>{plan.featuresList?.length || 0} features</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant='outline'>{getSubscriptionCount(plan.id)} active</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant={plan.isActive ? "default" : "secondary"}>{plan.isActive ? "Active" : "Inactive"}</Badge>
                    </TableCell>
                    <TableCell className='text-right'>
                      <div className='flex justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100'>
                        ...
                        <Button variant='ghost' size='icon' onClick={() => handleDuplicatePlan(plan)}>
