...
export default function SalesPage() {
  ...
  // Export to CSV
  const exportToCSV = () => {
    const headers = ["Date", "Merchant", "Email", "Plan", "Amount", "Type", "Status", "Transaction ID"];
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>Track all subscription sales and revenue</p>
        </div>
        <div className='flex gap-2'>
          ...
          <Button variant='outline' onClick={exportToCSV} disabled={filteredSales.length === 0}>
          ...
          <CardContent className='pt-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-3xl font-bold'>{formatAmount(stats?.totalRevenue || 0)}</p>
              </div>
          ...
          <CardContent className='pt-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-3xl font-bold'>{stats?.byType?.renewal || 0}</p>
              </div>
              ...
              <TableBody>
                {filteredSales.map((sale) => (
                  <TableRow key={sale.id}>
                    ...
                    <TableCell>
                      <div>
                        <p className='font-medium'>{sale.merchantName || sale.merchantId}</p>
                        {sale.merchantEmail && <p className='text-xs text-muted-foreground'>{sale.merchantEmail}</p>}
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge variant='outline'>{getBillingLabel(sale.billingCycleMonths)}</Badge>
                    </TableCell>
                    <TableCell>
                      <span className='font-bold text-green-600'>{formatAmount(sale.amount)}</span>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge className={getStatusBadgeClass(sale.status)}>{sale.status}</Badge>
                    </TableCell>
                    <TableCell>
                      <code className='text-xs bg-muted px-1 py-0.5 rounded'>{sale.transactionId?.slice(0, 12) || "-"}...</code>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
