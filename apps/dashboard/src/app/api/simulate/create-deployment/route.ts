...
export async function POST(request: Request) {
  try {
    ...
    const { merchantId, merchantName, merchantEmail, databaseName, customSubdomain } = body;

    // Check if Vercel deployment should be skipped
    const skipVercelDeployment = process.env.SKIP_VERCEL_DEPLOYMENT === "true" || process.env.DISABLE_VERCEL_DEPLOYMENT === "true";

    if (skipVercelDeployment) {
      console.log(`⏭️ [Simulate] SKIP_VERCEL_DEPLOYMENT is enabled - skipping Vercel deployment`);
    }
    ...
    if (!merchantId || !merchantName || !databaseName) {
      return NextResponse.json({ error: "Merchant ID, name, and database name are required" }, { status: 400 });
    }
    ...
    if (!actualMerchantEmail) {
      ...
      if (merchant) {
        ...
        console.log(`[Simulate] Retrieved merchant email from database: ${actualMerchantEmail}`);
      } else {
    ...
    if (!connectionString) {
      return NextResponse.json({ error: "Failed to get database connection string" }, { status: 500 });
    }
    ...
    if (skipVercelDeployment) {
      ...
      console.log(`⏭️ [Simulate] Creating mock deployment data (Vercel skipped)`);
      vercelProject = {
      ...
      console.log(`[Simulate] Creating Vercel project for merchant: ${merchantId}`);
      ...
      deployment = await deployToVercel(vercelProject.id, merchantId, databaseName, connectionString);
      ...
      deploymentUrl = currentDeployment.url || `${vercelProject.name}.vercel.app`;

      // Try to add the auto-generated subdomain to Vercel
      try {
        console.log(`[Simulate] Adding subdomain: ${subdomainToUse}`);
        domainResult = await addVercelSubdomain(vercelProject.id, subdomainToUse);
        subdomain = subdomainToUse;
        // Use the configured subdomain as deployment URL
        deploymentUrl = `https://${subdomainToUse}`;
        subdomainConfigured = true;
        console.log(`[Simulate] Subdomain added: ${domainResult.name} (verified: ${domainResult.verified})`);
        console.log(`[Simulate] Using Vercel DNS: ${domainResult.usingVercelDNS}, DNS Auto-configured: ${domainResult.dnsAutoConfigured}`);
        ...
        if (domainResult.usingVercelDNS && domainResult.dnsAutoConfigured) {
          console.log(`[Simulate] ✅ DNS automatically configured via Vercel DNS`);
          // Still provide DNS info for reference, but mark as auto-configured
          dnsInstructions = domainResult.dnsRecords || [];
        } else if (domainResult.dnsRecords && domainResult.dnsRecords.length > 0) {
          ...
          dnsInstructions = domainResult.dnsRecords.map((record: any) => ({
            ...
            name: record.name,
            value: record.value || domainResult.cnameValue || "cname.vercel-dns.com",
            ttl: 3600,
          }));
          console.log(`[Simulate] DNS instructions from domain result:`, dnsInstructions);
        } else {
          // Fetch DNS instructions from Vercel API if not in domain result
          console.log(`[Simulate] Fetching DNS instructions from Vercel API...`);
          try {
            dnsInstructions = await getDomainDNSInstructions(subdomainToUse);
            console.log(`[Simulate] DNS instructions retrieved from API:`, dnsInstructions);
            ...
            console.warn(`[Simulate] Failed to get DNS instructions: ${dnsError.message}`);
        ...
        console.warn(`[Simulate] Failed to add subdomain ${subdomainToUse}: ${error.message}`);
    ...
    const deploymentRecord: MerchantDeployment = {
      ...
      subdomain: subdomain,
      deploymentStatus: skipVercelDeployment ? "active" : currentDeployment.state === "READY" ? "active" : "pending",
    ...
    if (skipVercelDeployment) {
      console.log(`✅ [Simulate] Deployment record created (Vercel skipped): ${deploymentUrl}`);
    }

    // Create merchant user account in the merchant's database
    let merchantUser = null;
    let userEmail = actualMerchantEmail || `merchant_${merchantId}@${merchantName.toLowerCase().replace(/[^a-z0-9]/g, "")}.com`;
    let merchantPassword = "";

    try {
      // Use the actual merchant email from the merchant record
      // Generate password
      merchantPassword = `Merchant${merchantId.slice(-6)}!${Math.random().toString(36).slice(-4)}`;

      console.log(`[Simulate] Creating merchant user with email: ${userEmail} (from merchant record)`);
      ...
      const existingUser = await usersCol.findOne({ email: userEmail.toLowerCase() });
      ...
      if (!existingUser) {
        ...
        console.log(`[Simulate] ✅ Created merchant user account with merchantId: ${merchantId}, email: ${userEmail}`);
      } else {
        // User already exists - update to add merchantId if missing
        if (!existingUser.merchantId) {
          await usersCol.updateOne({ _id: existingUser._id }, { $set: { merchantId: merchantId, updatedAt: new Date().toISOString() } });
          console.log(`[Simulate] Updated existing user with merchantId: ${merchantId}`);
        }
        merchantUser = {
    ...
    return NextResponse.json({
      ...
      deployment: {
        ...
        status: currentDeployment.state === "READY" ? "active" : currentDeployment.state.toLowerCase(),
    ...
    return NextResponse.json({ error: error.message || "Failed to create deployment" }, { status: 500 });
  }
}
