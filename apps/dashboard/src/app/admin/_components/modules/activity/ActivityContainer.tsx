...
function ActionBadge({ action }: { action: string }) {
  ...
  return (
    <Badge
      className={`${colors[action.toLowerCase()] || "bg-gray-500"} capitalize`}
    >
      {action.replace(/_/g, " ")}
    </Badge>
  );
...
export default function ActivityContainer() {
  ...
  const loadLogs = async (page = 1) => {
    ...
    try {
      ...
      // Normalize meta to match PaginationInfo type (handle both totalPage and totalPages)
      const meta = result.meta;
      const normalizedMeta: PaginationInfo = meta
        ? {
            page: meta.page || 1,
            limit: meta.limit || 20,
            total: meta.total || 0,
            totalPages:
              (meta as any).totalPages || (meta as any).totalPage || 0,
          }
        : { page: 1, limit: 20, total: 0, totalPages: 0 };

      setPagination(normalizedMeta);
    } catch (error: any) {
  ...
  const handleClearOldLogs = async () => {
    ...
    try {
      const data = await api.delete<{ deletedCount: number }>(
        "activity-logs?olderThanDays=90"
      );
      toast.success(`Deleted ${data.deletedCount || 0} old logs`);
      loadLogs();
    } catch (error: any) {
          ...
          {loading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Activity logs are automatically recorded when actions are
                performed
              </p>
            </div>
          ) : (
