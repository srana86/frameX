  ...
  if (step.status === "running") {
    return (
      <div className={`${baseClasses} h-12 w-12`}>
        ...
        <div
          className={`relative flex h-12 w-12 items-center justify-center rounded-full ${step.color} shadow-lg`}
        >
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (isDeploying) {
        e.preventDefault();
        e.returnValue =
          "Deployment is in progress. Are you sure you want to leave?";
    ...
    const pollStatus = async () => {
      try {
        const data = await api.get<{ status: string; url?: string }>(
          `simulate/deployment-status?deploymentId=${deploymentId}`
        );
        const status = data.status.toUpperCase();
        setDeploymentStatus(status);
        updateStep("7", "running", `Deployment status: ${status}...`, {
          status,
          url: data.url,
          deploymentId,
        });
  ...
  const simulateFlow = async () => {
    ...
    try {
      ...
      updateStep(
        "1",
        "completed",
        `Plan: ${selectedPlan?.name} (${formatAmount(
      ...
      updateStep("3", "completed", `Subscription active`, {
        subscriptionId,
      ...
      try {
        ...
        let errorMessage = error?.message || "Failed to create database";
        if (error?.data?.details) {
          const details =
            typeof error.data.details === "string"
              ? error.data.details.split("\n").join(" | ")
              : error.data.details;
      ...
      updateStep(
        "5",
        "completed",
        `Created ${dbData?.collections || 10} collections`
      );
      ...
      if (isVercelSkipped) {
        ...
        setLoading(false);
        const deploymentUrl =
          deployData.deployment.url || "http://localhost:3000";
        updateStep("7", "completed", `Deployment ready (Local mode)`, {
          deploymentUrl,
          status: "active",
          skipped: true,
        });
      ...
      if (
        deployData.deployment.status === "active" ||
        deployData.deployment.status?.toUpperCase() === "READY"
      ) {
        setIsDeploying(false);
        setLoading(false);
        const deploymentUrl =
          deployData.deployment.url || deployData.deployment.vercelUrl;
      ...
      setSimulationResult({
        ...
        databaseName: dbData.databaseName,
        deploymentUrl:
          deployData.deployment.url ||
          deployData.deployment.vercelUrl ||
          "Deploying...",
        vercelUrl: deployData.deployment.vercelUrl,
        projectId: deployData.project.id,
          ...
          <CardContent className="flex items-center gap-4 pt-6 relative">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                Please stay on this page until the deployment is complete.
              </p>
            </div>
            <Badge className="bg-yellow-500 text-yellow-950 font-mono">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {selectedPlan.description || "Monthly subscription"}
                      </p>
                    </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <ul className="text-xs text-muted-foreground space-y-1.5">
                    ...
                    <li className="flex items-center gap-2">
                      <Rocket className="h-3 w-3 text-orange-500" /> Deploys a
                      live Vercel instance
                    </li>
                    <li className="flex items-center gap-2">
          ...
          <Card className="h-full overflow-hidden">
            <CardHeader className="pb-4 bg-linear-to-br from-muted/50 to-transparent">
              ...
              <div className="flex items-center gap-2 mt-4">
                <PhaseHeader
                  phase="setup"
                  isActive={phaseStatus.setup?.active}
                  isCompleted={phaseStatus.setup?.completed}
                />
                <ChevronRight className="h-4 w-4 text-muted-foreground/50" />
                <PhaseHeader
                  phase="provision"
                  isActive={phaseStatus.provision?.active}
                  isCompleted={phaseStatus.provision?.completed}
                />
                <ChevronRight className="h-4 w-4 text-muted-foreground/50" />
                <PhaseHeader
                  phase="deploy"
                  isActive={phaseStatus.deploy?.active}
                  isCompleted={phaseStatus.deploy?.completed}
                />
              </div>
            </CardHeader>
          ...
          {simulationResult ? (
            <>
              ...
              {simulationResult.deploymentUrl &&
                simulationResult.deploymentUrl !== "Deploying..." && (
                  <Card className="overflow-hidden">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="rounded-xl border bg-muted/30 p-3">
                      ...
                      <p className="font-mono text-xs truncate">
                        {simulationResult.databaseName}
                      </p>
                    </div>
                    <div className="rounded-xl border bg-muted/30 p-3">
                      <p className="text-xs text-muted-foreground mb-1">Plan</p>
                      <p className="text-sm font-semibold">
                        {simulationResult.plan?.name}
                      </p>
                    </div>
                    <div className="rounded-xl border bg-muted/30 p-3">
                      <p className="text-xs text-muted-foreground mb-1">
              ...
              {/* DNS Instructions */}
              {simulationResult.dnsInstructions &&
                simulationResult.dnsInstructions.length > 0 &&
                !simulationResult.dnsAutoConfigured && (
                  <Card className="border-amber-500/50 overflow-hidden">
                    ...
                    <CardContent className="space-y-3">
                      {simulationResult.dnsInstructions.map(
                        (dns: any, index: number) => (
                          <div
                            key={index}
                            ...
                            <div className="flex items-center justify-between text-sm">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    copyToClipboard(dns.value, `dns-${index}`)
                                  }
                                >
            ...
            <Card className="h-full overflow-hidden">
              <CardContent className="flex flex-col items-center justify-center h-full py-16 text-center">
                ...
                <p className="text-sm text-muted-foreground max-w-[240px]">
                  Configure your simulation parameters and click Launch to begin
