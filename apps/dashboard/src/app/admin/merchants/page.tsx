...
export default function MerchantsPage() {
  ...
  const loadMerchants = async () => {
    setLoading(true);
    try {
      const [merchantsData, plansData, subscriptionsData] = await Promise.all([
  ...
  const handleAssignPlan = async () => {
    ...
    try {
      ...
      toast.success(
        `${selectedPlan?.name} plan assigned to ${assigningMerchant.name}!`
      );
  ...
  const handleCreateMerchant = async () => {
    ...
    try {
      await api.post("merchants", newMerchant);
  ...
  const handleUpdateMerchant = async () => {
    ...
    try {
      await api.put(`merchants/${editingMerchant.id}`, editingMerchant);
  ...
  const handleDeleteMerchant = async (
    ...
    if (
      !confirm(
        `Are you sure you want to delete "${merchantName}"? This action cannot be undone.`
      )
    ) {
      return;
    }
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage all merchant accounts and their deployments
          </p>
        </div>
          ...
          <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Add a new merchant account to the system
                </DialogDescription>
              </DialogHeader>
                          ...
                          if (!sub) {
                            return (
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleOpenAssignPlan(merchant)}
                                className="gap-1"
                              >
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                          ...
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              handleDeleteMerchant(merchant.id, merchant.name)
                            }
                          >
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm">
                    {viewingMerchant.phone || "Not provided"}
                  </p>
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">
                  ...
                  <p className="text-sm">
                    {viewingMerchant.customDomain || "Not configured"}
                  </p>
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">
                ...
                {viewingMerchant.createdAt && (
                  <div className="space-y-1">
                    ...
                    <p className="text-sm">
                      {new Date(viewingMerchant.createdAt).toLocaleString()}
                    </p>
                  </div>
                )}
              </div>
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {assigningMerchant
                ? `Assign or update subscription for ${assigningMerchant.name}`
          ...
          {assigningMerchant && (
            <div className="space-y-6">
              ...
              {(() => {
                const currentSub = getMerchantSubscription(
                  assigningMerchant.id
                );
                if (currentSub) {
