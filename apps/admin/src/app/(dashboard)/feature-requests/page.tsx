...
export default function FeatureRequestsPage() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const data = await api.get<FeatureRequest[]>("feature-requests");
      setRequests(Array.isArray(data) ? data : []);
    } catch (error: any) {
      console.error(error);
      toast.error(error?.message || "Failed to load feature requests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateRequest = async (
    id: string,
    patch: Partial<Pick<FeatureRequest, "status" | "priority">>
  ) => {
    startTransition(async () => {
      try {
        const data = await api.patch<FeatureRequest>(
          `feature-requests/${id}`,
          patch
        );
        setRequests((prev) =>
  ...
  const getStatusBadge = (status: FeatureRequestStatus) => {
    ...
    return (
      <Badge className={cn("capitalize", map[status])}>
        {status.replace("_", " ")}
      </Badge>
    );
  };
  ...
  const getPriorityBadge = (priority: FeatureRequestPriority) => {
    ...
    return (
      <Badge className={cn("capitalize", map[priority])}>{priority}</Badge>
    );
  };
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div className="flex gap-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setPriorityFilter(v as FeatureRequestPriority | "all")
                }
              >
                <SelectTrigger className="w-[160px]">
                  <Filter className="mr-2 h-4 w-4" />
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredRequests.map((req) => (
                        <TableRow
                          key={req.id}
                          className="align-top hover:bg-accent/30 transition-colors"
                        >
