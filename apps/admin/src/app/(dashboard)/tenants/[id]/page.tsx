...
export default function TenantDetailPage({
  ...
  const [data, setData] = useState<TenantFull | null>(null);
  const [loading, setLoading] = useState(true);

  const loadData = async () => {
    setLoading(true);
    try {
      const tenantData = await api.get<TenantFull>(
        `tenants/${resolvedParams.id}/full`
      );
      setData(tenantData);
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        "Are you sure you want to delete this tenant and all associated data? This action cannot be undone."
      )
    ) {
      return;
    }
    ...
    try {
      await api.delete(`tenants/${resolvedParams.id}`);
  ...
  const { tenant, subscription, plan, deployment, database } = data;
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="grid gap-4 sm:grid-cols-2">
                <InfoItem
                  label="Tenant ID"
                  value={<code className="text-xs">{tenant.id}</code>}
                />
                <InfoItem label="Name" value={tenant.name} icon={Building2} />
                <InfoItem label="Email" value={tenant.email} icon={Mail} />
                <InfoItem label="Phone" value={tenant.phone} icon={Phone} />
                <InfoItem
                  label="Status"
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {!deployment?.deploymentUrl && !tenant.customDomain && (
            ...
            <CardContent>
              {subscription && plan ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This tenant doesn&apos;t have an active subscription
                  </p>
              ...
              {deployment ? (
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-semibold capitalize">
                        {deployment.deploymentType.replace("_", " ")}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        {deployment.deploymentUrl}
                      </p>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <InfoItem
                      label="Deployment ID"
                      value={<code className="text-xs">{deployment.id}</code>}
                    />
                    <InfoItem
                      label="Provider"
                      value={deployment.deploymentProvider || "—"}
                      icon={Server}
                    />
                    <InfoItem
                      label="Subdomain"
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This tenant doesn&apos;t have a deployment configured
                  </p>
                  <Link href="/simulate">
              ...
              {database ? (
                <div className="space-y-6">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <InfoItem
                      label="Database ID"
                      value={<code className="text-xs">{database.id}</code>}
                    />
                    <InfoItem
                      label="Database Name"
                      value={<code>{database.databaseName}</code>}
                      icon={Database}
                    />
                    <InfoItem
                      label="Type"
                      value={
                        database.useSharedDatabase ? "Shared" : "Dedicated"
                      }
                      icon={HardDrive}
                    />
                    <InfoItem
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This tenant doesn&apos;t have a database configured
                  </p>
                  <Link href="/simulate">
            ...
            <CardContent>
              {tenant.settings ? (
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <InfoItem
                    label="Brand Name"
                    value={tenant.settings.brandName}
                    icon={Building2}
                  />
                  <InfoItem
                    label="Currency"
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This tenant hasn&apos;t configured any settings yet
                  </p>
                </div>
              )}
            </CardContent>
